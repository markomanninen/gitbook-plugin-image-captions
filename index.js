...
function processImages(that) {

  var options = that.options.pluginsConfig['image-captions'] || {};

  var imagePageWrapper = function(page) {
    var collectImages = function(section) {
      ...
      $('img').each(function(i, elem) {
        ...
        if (caption) {
          var key = page.progress.current.level+'.'+(i+1);
          images[key] = {
            // page image order
            index: i,
            // image src
            ...
            // page level
            page_level: page.progress.current.level,
...
function processCaptions(page, that) {
  ...
  var insertCaptions = function(section) {
    ...
    $('img').each(function(i, elem) {
      ...
      var wrapImage = function(caption) {
        ...
        var result = createCaption(key, caption, options, 'caption', page.progress.current.level, i, nro);
